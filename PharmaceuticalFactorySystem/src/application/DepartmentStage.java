...
public DepartmentStage () {
	...
	update.setOnAction(e->{
		...
		else managerId1.setText(selected.getManagerId()+"");
		...
		try (PreparedStatement stmt = Main.conn.prepareStatement(getDeptEmpCount)) {
		    ...
		    if (rs.next()) {
		        ...
		        if (count == 0) managerId1.setEditable(false);
		    }
		...
		update.setOnAction(e1->{
			...
			Main.validAlert("Department Updated", "Department with id " + selected.getDeptId() + " was updated successfully");
	...
	VBox all = new VBox (10 , addBox , buttons , deptTable);
